...
def test_worker_config_is_the_automatic_adapter_opt_in(
    ...
    monkeypatch.setenv(activation, "1")
    monkeypatch.setenv("LM_RESILIENCY_TORCHRUN_WORKER_CONFIG", "/stale/worker.toml")
    ...
    try:
        ...
        assert "LM_RESILIENCY_TORCHRUN_WORKER_CONFIG" not in os.environ
        assert os.environ["LM_RESILIENCY_TORCHRUN_RUN_ID"] == "explicit"
    finally:
