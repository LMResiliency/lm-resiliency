...
class RestartAckStateReader:
    ...
    def read(
        ...
        node_id = receipt.acknowledgement.node_id
        registration_key = agent_registration_key(self._run_id, node_id)
        for _ in range(_MAX_READ_ATTEMPTS):
            opened = self._read_open()
            registration_entry = self._store.get(registration_key)
            registration_history = self._store.get_history(registration_key)
            registration_has_history = self._store.has_history(registration_key)
            lease_history = self._read_lease_history()
            if (
                opened != self._read_open()
                or registration_entry != self._store.get(registration_key)
                or registration_history != self._store.get_history(registration_key)
                or registration_has_history != self._store.has_history(registration_key)
            ...
            return self._authenticate(
                ...
                registration_has_history=registration_has_history,
    ...
    def _authenticate(
        ...
        *,
        ...
        registration_entry: ControlStoreEntry | None,
        registration_history: tuple[ControlStoreEntry, ...],
        registration_has_history: bool,
        ...
        if not registration_has_history:
            raise RestartAckStateCorrupt("live agent registration has no durable history")
        if not registration_history or registration_history[-1] != registration_entry:
            raise RestartAckStateCorrupt(
                "live agent registration is absent from its durable history"
            )
        registration = _decode_registration(
            registration_entry,
            run_id=self._run_id,
            node_id=receipt.acknowledgement.node_id,
        )
        if registration != receipt.authenticated_registration:
            raise RestartAckStateRegistrationLost(
                "authenticated agent registration is no longer current"
            )
...
def _decode_registration(
    ...
    *,
    ...
    try:
        ...
        raise RestartAckStateCorrupt("persisted agent registration is malformed") from error
    if entry.value != record.to_json():
        raise RestartAckStateCorrupt("persisted agent registration is noncanonical")
    identity = record.agent_identity
    if identity.run_id != run_id or identity.node_id != node_id:
        raise RestartAckStateCorrupt("persisted agent registration belongs to another run or node")
    ...
    if entry.guard_key is not None:
        raise RestartAckStateCorrupt(
            "persisted agent registration unexpectedly carries guard provenance"
        )
